...
export default function HomeFaxStandardFindingCard({ issue, apiBaseUrl, onRefresh,
  ...
  const rawCurrentStatus = pickFirst(issue.current_status, issue.status);
  ...
  const baselineLocked = ["yes", "true", "1", "locked"].includes(
    String(issue.baseline_locked || issue.baselineLocked || "").toLowerCase()
  );

  const adminReviewStatus = pickFirst(issue.admin_review_status, "pending");
  const adminImageDecision = pickFirst(issue.admin_image_decision, "pending");
  const imageMatchStatus = pickFirst(issue.image_match_status, "suggested");
  const finalApprovalStatus = pickFirst(issue.final_approval_status, "not_approved");
  const verifiedImageUrl = pickFirst(issue.verified_image_url, "");
