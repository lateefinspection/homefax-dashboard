...
function AdminImagePreviewCard({
  ...
  apiBaseUrl,
  note,
  emptyText = "No image available yet.",
}) {
  const resolvedUrl = imageUrl ? getImageUrl(apiBaseUrl, imageUrl) : "";
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
      ...
      {resolvedUrl ? (
        <div>
          <a href={resolvedUrl} target="_blank" rel="noreferrer">
            <img
              src={resolvedUrl}
              alt={label}
              className="h-44 w-full bg-slate-900 object-cover transition hover:opacity-90"
            />
          </a>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-slate-950 hover:bg-slate-200"
            >
              Open actual image
            </a>
        ...
        <div className="px-3 py-6 text-sm font-semibold text-slate-400">
          {emptyText}
        </div>
      )}
    </div>
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div>
                    <div className="text-sm font-black text-white">
                      Homeowner / Admin Image Verification Context
                    </div>
                    <div className="mt-1 max-w-3xl text-xs font-semibold leading-5 text-slate-300">
                      Admin should verify the actual photo evidence before final approval lock. Review the homeowner image decision, inspect the primary evidence image, and confirm the saved verified image.
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-slate-100">
                      Homeowner image: {formatDecisionLabel(issue.homeowner_image_decision || "unreviewed")}
                    </span>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-slate-100">
                      Admin image: {formatDecisionLabel(adminImageDecision || "pending")}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-3 lg:grid-cols-2">
                  <AdminImagePreviewCard
                    ...
                    note="This is the primary evidence image the admin can verify for the finding."
                    emptyText="No primary image is available yet."
                  />

                  <AdminImagePreviewCard
                    label="Verified Image Saved"
